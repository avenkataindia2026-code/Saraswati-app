 ...
 */
function handleButtonClick(el, label) {
    if (label === 'About') toggleAbout(true);
}
...
window.addEventListener('load', () => {
    ...
    if (canvas) {
        canvas.addEventListener('mousedown', (e) => {
            if (typeof triggerRipple === 'function') triggerRipple(e);
        });
    }

    const closeBtn = document.getElementById('close-overlay');
    if (closeBtn) closeBtn.onclick = () => toggleAbout(false);
...
function updateStory(langId) {
    ...
    const themes = {
        'lang-ta': { content: TA_ABOUT, color: '#D9791A', bg: '#FFF5E6', font: "'Tiro Tamil', serif" },        // Saffron on peach
        'lang-en': { content: EN_ABOUT, color: '#06038D', bg: '#FFFFFF', font: "'Quicksand', sans-serif" },         // blue on white
        'lang-hi': { content: HI_ABOUT, color: '#138808', bg: '#F0F9F0', font: "'Tiro Devanagari Hindi', serif" }   // Green on mint
    };
...
const navStates = {
    main: {
        ...
        links: [
            ...
            { id: 'nav-about', text: 'About the Project', href: '#', type: 'action', fx: showAbout }
        ]
    },
    community: {
        ...
        links: [
            ...
            { id: 'nav-comm', text: 'Back to Main Page', href: '#', type: 'state', target: 'main' },
